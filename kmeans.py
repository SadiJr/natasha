...
def main(dataset, stop_words, lem, stem, min_df, max_df, k_start, k_stop, k_step, interaction_start, interaction_stop,
         interaction_step, n_init_start, n_init_stop, n_init_step, matrix, n_components, random_state, disable_plots,
         output_dir):
    ...
    if matrix == 'original':
        loop(df, X, k_start, k_stop, k_step, interaction_start, interaction_stop, interaction_step, n_init_start,
             n_init_stop, n_init_step, random_state, None, output_dir, disable_plots)
    elif matrix == 'all':
        ...
        loop(df, Y_pca, k_start, k_stop, k_step, interaction_start, interaction_stop, interaction_step, n_init_start,
             n_init_stop, n_init_step, random_state, "PCA", output_dir, disable_plots)
        loop(df, Y_tsne, k_start, k_stop, k_step, interaction_start, interaction_stop, interaction_step, n_init_start,
             n_init_stop, n_init_step, random_state, "TSNE", output_dir, disable_plots)
    elif matrix == 'pca':
...
def loop(df, X, k_start, k_stop, k_step, interaction_start, interaction_stop, interaction_step, n_init_start,
         n_init_stop, n_init_step, random_state, reduction_type, output_dir, disable_plots):
    pd.DataFrame(columns=['K', 'max_iter', 'n_init', 'random_state', 'reduction_type', 'silhouette_euclidian',
    ...
    for i in np.arange(start=k_start, stop=k_stop, step=k_step):
        for j in np.arange(start=interaction_start, stop=interaction_stop, step=interaction_step):
            for k in np.arange(start=n_init_start, stop=n_init_stop, step=n_init_step):
                kmeans(df, X, i, j, k, random_state, reduction_type, output_dir, disable_plots)


def kmeans(df, X, K, max_iter, n_init, random_state, reduction_type, output_dir, disable_plots):
    start = timer()
    ...
    pd.DataFrame(data=[[K, max_iter, n_init, random_state, reduction_type, silhouette_euclidian, silhouette_cosine,
                        silhouette_manhattan, calinski, davies, duration]],
                 columns=['K', 'max_iter', 'n_init', 'random_state', 'reduction_type', 'silhouette_euclidian',
                          'silhouette_cosine', 'silhouette_manhattan', 'calinski', 'davies', 'time'])\
    ...
    df.to_csv(f'{output_dir}{K}-{max_iter}-{n_init}-{random_state}-{reduction_type}.csv')
...
def parse_args():
    ...
    parser.add_argument('--k-start', help='the default K number to start clustering', type=int, default=2)
    parser.add_argument('--k-stop', help='the default K number to stop clustering', type=int, default=10)
    parser.add_argument('--k-step', help='the default step number to increment in the K value', type=int,
                        default=1)
...
if __name__ == '__main__':
    try:
        ...
        main(args.dataset, args.stop_words, args.lem, args.stem, args.min_df, args.max_df, args.k_start, args.k_stop,
             ...
             args.n_init_stop, args.n_init_step, args.matrix, args.n_components, args.random_state, args.disable_plots,
             args.output_dir)
        logger.info('Finish KMeans algorithm run.')
    except Exception:
        logger.error('Error when running KMeans algorithm: ')
