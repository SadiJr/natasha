...
def agglo(df, X, terms, K, link, affinity):
    start = timer()

    agglomerative = AgglomerativeClustering(n_clusters=K, linkage=link, affinity=affinity)
    X_t = agglomerative.fit_predict(X.toarray())
    end = timer()

    description = f'Metrics of Agglomerative with {K}, linkage {link} and affinity {affinity} are: '
    ...
    try_result = pd.DataFrame(data=[[K, link, affinity, silhouette_euclidian, silhouette_cosine,
